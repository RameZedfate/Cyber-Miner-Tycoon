...
description: 排出 @fang__0914 一整個月的 IG 選題表，每一格都有主題、形式、切角與視覺方向。當使用者說「這個月要發什麼」「幫我排選題」「下個月的內容」時使用。產出會給 caption-writer 與 reels-script-writer 接手。
---

## ⚙️ Spark 執行環境轉接（Gemini Spark 專用，優先於下方原文）

這份技能原本寫給有檔案系統的環境（Claude Code）。在 Spark 執行時，以下對應優先：

| 原文寫的 | 在 Spark 要做的事 |
| --- | --- |
| 讀 `context/brand-style.md` | 呼叫 `/adam-brand-context` 技能；若使用者已把品牌檔放進雲端硬碟，改讀該檔 |
| 讀 `context/content-calendar.md` | 讀雲端硬碟 `AI-100天/content-calendar` 文件；沒有就先問使用者這個月主題 |
| 寫入 `outputs/<平台>/xxx.md` | 建立 Google 文件，命名 `<平台>-<日期>-<主題>`，放進雲端硬碟 `AI-100天/outputs/`；使用者沒授權雲端硬碟時，直接把完整內容輸出在對話裡 |
| 「client / SMB 客戶」 | 一律指 Adam 本人的個人品牌 `@fang__0914`，不是外部客戶 |
| 需要 Nano Banana / Blotato / Firecrawl / SerpApi MCP 的步驟 | Spark 沒有這些，跳過該步驟並明講跳過了什麼，不要假裝做了 |
| 產出語言 | **一律繁體中文（台灣用語）**，工具名保留英文原文 |

**紅線（違反就停下來問）**：任何產出都不得出現保險客戶姓名、身分證、保單號碼、
永達內部系統畫面。保險相關題材一律用虛構案例。

...
- **航線範圍**：目前只抓 **日本 / 韓國 / 越南 / 泰國**（其餘國家、含馬尼拉/香港等都不顯示）。要增減國家改 `index.html` 的 `ALLOWED_COUNTRIES`、並在 `COUNTRY_OF` 補對應機場代碼。
- **傳統航空專區**：底下另開 **華航 / 長榮 / 星宇** 三欄，各列「來回」最便宜前 3 名、**兩個月內出發**。
- 卡片照片以**國家代表照**呈現（日本=伏見稻荷鳥居、韓國=釜山廣安大橋、越南=峴港黃金橋、泰國=天燈節），可在 `COUNTRY_TITLE` 換條目。
...
### 卡片上的城市照片（自動，來源：維基百科）
每張卡片的標題區會**自動抓該目的地的代表照片**（像 Trip.com 那樣），任何國家都適用。
- 來源是**維基百科的條目代表圖**（穩定、免金鑰），由瀏覽器即時抓取並**快取到你裝置**，所以越用越快。
- **只有確認抓到圖才會放上卡片**，抓不到就維持漂亮的地區底色漸層 → **不會出現破圖或亂入的怪照片**。
- 想換某個城市要抓的主題：改 `WIKI_TITLE`（key 用機場代碼、值用維基條目名），例如：
  ```js
  const WIKI_TITLE = {
    HKG:"Victoria Harbour",   // 香港 → 維多利亞港
    ICN:"N Seoul Tower",      // 首爾 → 首爾塔
    OKA:"Kabira Bay",         // 沖繩 → 川平灣海景
    KIX:"Osaka Castle"        // 大阪 → 大阪城
  };
  ```
- **想某城市釘死成「你自己的一張照片」**（最高品質、固定不變）：在 `DEST_IMAGE` 填機場代碼＋圖網址，會優先用你的圖。
  ```js
  const DEST_IMAGE = { NRT:"https://……你挑的東京照.jpg" };
  ```
- 照片若想全部重抓：清除瀏覽器該網站的 localStorage（或等快取，名稱 `cf_photos`）。


> 另外：**飛台灣國內線（金門 KNH、澎湖 MZG、花蓮、台東…）不會顯示**，只留出國的便宜票。要改清單看 `TW_DOMESTIC`。

...
### 換卡片上的地區照片（日本/韓國/東南亞/港澳）
`index.html` 裡有一個 `REGION_IMAGE`，每個地區可放一張背景照（卡片標題區會用它）：
```js
const REGION_IMAGE = {
  jp:"https://……日本的圖.jpg",   // 例如鳥居那張
  kr:"", sea:"", hk:""            // 留空＝只用底色漸層（也很好看）
};
```
- 想換日本卡片的圖（例如你喜歡的鳥居照），把 `jp` 的網址換掉即可；其他地區同理。
- 圖載入失敗會自動退回底色漸層，不會壞版。

...
### 卡片上的城市照片（自動）
每張卡片的標題區會**依目的地名稱自動抓一張相符的特色照片**（像 Trip.com 那樣），任何國家都適用，不用你一張張設定。
- 抓圖邏輯在 `cardBg()`：用目的地英文名＋地區關鍵字（例如 `Tokyo,Japan`）抓一張鎖定的照片。
- 圖萬一載入失敗，會自動退回**地區底色漸層**，不會破版。
- **想指定某個城市用「你自己的照片」**（最高品質）：在 `DEST_IMAGE` 填上機場代碼＋網址，例如：
  ```js
  const DEST_IMAGE = {
    NRT: "https://……你挑的東京照.jpg",   // 東京
    ICN: "https://……首爾照.jpg"          // 首爾
  };
  ```
  有填的就優先用你的圖；沒填的就走自動抓圖。
- 想換自動抓圖的風格關鍵字，改 `REGION_KW`。

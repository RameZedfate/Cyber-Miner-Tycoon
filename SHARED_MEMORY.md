...
- 2026-08-10：**第二次實案驗證，理賠申請書再多三家：遠雄、台灣人壽、國泰個險。**
  ...
  - ⚠️ **國泰個險 300002 是兩頁一組，交付要 2 頁**（本文 300002 + 附件 00016），
    跟學團險 303002/303004 同一個模式。**我一開始誤判成 1 頁，是使用者抓出來的。**
    戶名／受款人身分證／金融機構／分行／帳號全在附件那頁，本文頁根本沒有帳戶欄位。
    → 教訓：**本文頁條碼列印了兩個編號（`*300002*  *00016*`）就是「還有附件」的訊號**，
    看到兩個編號要主動問使用者要第二頁，不要只交本文頁。
    這次使用者只給本文頁，附件版面**尚未驗證**，拿到要當 development mode 重測。

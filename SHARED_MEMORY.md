...
- 2026-08-10：**第二次實案驗證，理賠申請書再多三家：遠雄、台灣人壽、國泰個險。**
  - 使用者一次上傳「三家空白表單合併成一份 PDF」（遠雄 11501 / 台壽 CA03 2025.02 /
    國泰 300002 個險 115.08），三張都填好交付。
  - 引擎改版：`claim_forms.py` 新增 `source_page`（座標不含頁索引）、`cells`（不等寬逐格）、
    `boxes`（自動斷行縮字級），並回傳逐欄報告、有 warning 就 exit 1。舊三家版面不受影響（18 tests 全過）。
  - ⭐ **使用者新教的句型（已寫進 contract）：門診手術案件**用
    `因{病名}，於{手術日}至{科別}門診行{手術名稱}，於{回診日}回診拆線`，
    沒回診就只留前半句。**住院型「入院／出院」句型不可硬套在門診手術上。**
    → 這補掉了 2026-07-29 記的「門診手術會直接停住」那個洞。
    仍沒句型的是：**純住院無手術、慢性病回診**。
  - ⭐ **使用者確認：疾病案件不填職業／工作內容**（遠雄「工作內容」、台壽「事故時職業及工作內容」、
    國泰「工作內容」都屬意外續填區，一律留空）。
  - ❗ **踩到的雷：`claim_overlay_layout.py` 的 `farglory` 版面對不上真實遠雄 11501 版**
    （身分證逐格區起點差約 12pt＝整排錯一格）。遠雄改走 `claim_forms.py`。
    **推論：上游那七家「未驗證座標」不能信，第一次用一定要拿真表單重測。**
  - ❗ **遠雄帳號格印刷本身不等寬**（14 格裡第 10 格特寬、第 11 格特窄），
    等分計算會整排偏掉，所以才加了 `cells` 用實測邊界。這種事只有渲染放大看才會發現。
  - ⚠️ **國泰個險是兩頁一組，交付 2 頁：本文 300002 + 附件 300003（保險金給付方式）**，
    跟學團險 303002/303004 同一個模式。**我一開始誤判成 1 頁，是使用者抓出來的**，
    附件補進來後已完成並驗證。戶名／身分證／金融機構分行／帳號全在附件那頁。
    → 教訓一：**判斷「還缺附件」要看這張表有沒有「錢要匯去哪」的欄位**，
    沒有領取方式／帳戶欄位就一定還有附件頁，主動跟使用者要。
    → 教訓二：**不要用條碼列的第二組號碼猜附件編號**。本文印 `*300002* *00016*`、
    附件印 `*300003* *00014*`，第二組是另一套內部碼，我照著猜成「附件是 00016」猜錯了。
  - 引擎再擴充：`--form` 可重複指定（本文與附件分兩個檔掃進來不必先自己合併），
    版面新增 `source_pages` 清單格式（座標第一個元素是 slot，實際頁由清單決定）。
  - ⚠️ 使用者又直接把真實個資貼進對話（姓名／身分證／帳號／地址），
    違反本檔第 35 行紅線。案件 JSON 與成品 PDF 都只留暫存區沒進 git，技能資料夾已掃過無個資。

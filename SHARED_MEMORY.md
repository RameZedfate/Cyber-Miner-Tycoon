...
- 2026-07-27：建立**數字人 Reels 製作流程** `reels/REELS_PIPELINE.md`（全部單價/規格皆已實測）。
  ...
  - 技術路線（已驗證）：
    ...
    - 聲音 = ⭐**已定案：使用者本人錄音，直接當 `wan2_7` 的嘴型驅動源，不做克隆**（0 點）。
      - 原因：平台預設 voice **全部是英文 voice**（Eric/John/Marcus…），無中文母語 voice，唸中文必有腔調問題；且使用者是保險業務，講話感染力是強項，AI 模仿反而降級。
      - ⚠️ **關鍵陷阱**：`wan2_7` 靠音檔驅動嘴型。**不可以**先出無聲影片再去剪映配音，那樣嘴型會對不上。音檔必須在生成當下就餵進去。
      - 備案（不想錄音時）：`qwen_audio_tts`（阿里巴巴，原生中文，可用 `instruction` 下語氣指令）+ 克隆 element，0.2 點/段。**不要**用 `seed_audio` + 英文預設 voice。
  ...
  - 出鏡比例已定案：**混合型**，45 秒中約 15 秒數字人出鏡（開頭 hook + 結尾 CTA），中間走 B-roll，約 39 點/支。
  - 待辦（只有使用者能做）：① 上傳 3-5 張首幀照片 ② 給文稿 ③ 依分鏡表自行錄音上傳。

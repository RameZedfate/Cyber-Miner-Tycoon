...
- 2026-08-05：**雲端 Claude 每次都是全新容器 —— 空白表單與案件資料都不會留下來。**
  ...
  - ✅ 同日後續：**使用者當場上傳合併空白表單（page 0 全球／page 1 三商美邦），雲端實跑成功。**
    版次由 PDF 內文自動確認（全球 `CLMN00600` 2026.03、三商 `CL106C`），座標與已驗證版面吻合。
    ➜ 結論：**雲端可行，但每個 session 都要重新上傳空白表單**，這是固定成本不是故障。
  - ⚠️ 意外案件若使用者只給短短一句經過（例如四個字），依規則**原文照填、不補寫**。
    這是刻意的，不是漏填。

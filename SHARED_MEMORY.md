...
- 2026-07-30：**自製技能 `/dm-outreach`（IG／FB 私訊開發客戶＋增員）。這是第一個本 repo 原創、無上游的技能。**
  - 需求來源：使用者要在 FB／IG **開發新客戶＋增員**，做法是主動私訊或回覆別人的限時動態，
    要「追蹤名單」＋「回覆方式怎麼寫才加分」。
  - **先查過 GitHub，沒有能用的現成技能。** 查到的都不合用：
    `moboutrig/instagram-claude-skill`（走 Graph API，但 API 冷開發根本做不到）、
    `FarzamHejaziK/claude-linkedin-assistant`（LinkedIn，B2B 求職）、
    `sales-skills/sales`（40+ 個都是 Clay／Gong／Snov 這類 B2B SaaS 工具包裝，跟一對一私訊無關）。
    **全部是英文＋B2B＋假設可以 API 自動化**，沒有一個處理台灣保險法規或增員。
  - ❗ **最重要的技術結論：IG 冷私訊在 Meta 沒有合規的 API 路徑。**
    Messaging API 只能在對方**主動互動後 24 小時內**回覆（留言／回限動／私訊／點廣告 CTA 才算，
    **按讚不算**）；自動群發是封號風險最高的行為，自動訊息速率上限 2024 年起已從
    每小時 5,000 降到 **200**。`HUMAN_AGENT` 標籤能延到 7 天但 Meta 明文禁止用在自動訊息。
    ➜ 所以這個技能**只草擬、不代發**。這不是保守，是唯一能長期做的做法
    （而且限動回覆的價值來自「你真的看了那則限動」，自動化必然退化成罐頭訊息）。
  - ❗ **發現 repo 是 `public`（已用 GitHub API 確認 `"private": false`）。**
    追蹤表因此設計成**代號制**（`P-001` 客戶線／`R-001` 增員線）＋去識別化標籤，
    真名對照表放 `context/private/roster.md`，**已加進 `.gitignore`**。
    ➜ 這條同時修補了一個既有風險：安全紅線寫了「個資不上 GitHub」，但之前沒有機制擋。
  - 技能內容：`SKILL.md`（5 個 Phase）＋ 3 份 reference ＋ 2 份 template。
    - `reply-playbook.md` ← **核心**，回答「怎麼回覆才加分」。四階段模型、限動回覆專章、
      5 個加分點、6 個扣分點、送出前檢查清單。
    - `recruit-playbook.md` ← 增員線（跟客戶線是**兩件事**：找的是「現況不滿」不是「風險缺口」，
      節奏慢一倍，搞砸的成本是**少一個朋友**）。
    - `compliance-guardrails.md` ← 平台／法規／個資三層紅線。
  - 🔑 **整套技能最重要的一句話：私訊的目標不是成交，是換場。**
    使用者線下銷售能力強，所以私訊的唯一 KPI ＝「約到 15 分鐘通話」，
    不是在私訊裡把保險講完（文字談保險必輸）。
  - 🔑 **第二重要：「內容養、私訊收」。** IG 的 100 天連載本身就是最好的前置 ——
    對方回你私訊前一定會點頭像看兩秒，那兩秒決定他要不要認真回。
    這也讓增員有了一般人沒有的素材（不用講「這行很好」，直接讓他看你每天在幹嘛）。
  - ⚠️ **待辦（只有使用者能做）：確認永達的內部社群規範**
    （社群帳號要不要報備？可否提及公司名？商品內容的審閱流程？）。
    確認前所有草稿走最保守版本。確認後寫進 `context/outreach-profile.md`。
  - ⚠️ 下一步：跑一次 `/dm-outreach` 做設定訪談建立 `context/outreach-profile.md`，
    然後拿一則**真實的限動回覆案例**測，看草稿像不像本人講話。

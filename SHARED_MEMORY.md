...
- 2026-09-06：**新增「富邦人壽 要保人變更 應備文件檢核表」PDF**（`outputs/insurance/fubon-policyholder-change/`）。
  - 產出：A4 兩頁的**單一 PDF** + 原始 `checklist.html`。做法：寫 HTML → headless Chromium `--print-to-pdf`，
    中文字型用系統內建 **WenQuanYi Zen Hei**（容器內沒有其他中文字型，繁體渲染正常）。這套流程可重複用在其他表單。
  - 內容：A 表單類 9 項、B 證件與證明類 7 項、C 贈與稅、D 流程與辦理管道、E 送件前 60 秒自檢 14 項。
    每項標示三級來源：**官網／法規明載｜實務常見｜視案件才要**，避免把業界慣例講成官方規定。
  - ⚠️ **雲端環境封鎖 `fubon.com`（egress proxy 403，curl 與 WebFetch 都不通）** → **抓不到官方空白表單 PDF**。
    這份是自製檢核表，**不是富邦官方文件**。要夾帶官方表單，須由使用者本機下載後放進 repo 或暫存區。
  - 要保人變更的重點知識（下次可直接用）：
    1. 三方簽名缺一不可：**原要保人＋新要保人＋被保險人**；簽名須與原留簽樣相符。
    2. **新要保人與被保險人須有保險利益**（保險法§16），附關係證明（戶籍謄本實務要 3 個月內）。
    3. 新要保人要重做 **CRS／FATCA 稅務居住者聲明＋KYC**；投資型另加風險屬性問卷。
    4. **扣款帳戶／信用卡要改成新要保人本人**，否則「原要保人代繳」又是一筆贈與。
    5. 贈與額＝**變更日的保單價值準備金**（不是已繳保費），**30 日內**申報，115 年度免稅額 **244 萬／贈與人／年**。
    6. 富邦官網特別提醒：**豁免保費附約、受益人、地址、電話**要檢視是否一併變更；海外簽名需駐外館處文件證明。
    7. 原要保人已身故 → 走**繼承**流程（除戶謄本、繼承人同意書），不是一般要保人變更。
  - 📌 註記：2026-07-30 那條「交付＝申請書本身，不附文件清單」的長期規則，**是針對理賠申請書的交付**。
    這次是使用者**主動要求**保全變更的應備文件清單，不衝突。
  - 這類「保全變更」需求現有 `insurance-claim-form-automation`（理賠）技能不涵蓋；若之後常做，可考慮擴成保全變更技能。

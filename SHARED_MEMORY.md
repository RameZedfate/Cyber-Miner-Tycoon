...
- 2026-08-03（同日稍晚）：🔴 **使用者決定停做「AI 挑戰100天」，改成數字人 podcast 短影音。**
  - 新方向：**主題 AI 應用、單支 30-60 秒、數字人講話形式**（不是螢幕錄影）。
  - 新規格與 10 支選題庫：`context/podcast-series.md`。
    第一支腳本：`outputs/scripts/podcast-01-ai-not-google.md`（「我一直在把 AI 當 Google 用」）。
  - ⚠️ **失效檔案**：`context/content-calendar.md` 整份作廢；
    `context/brand-style.md` 裡「Day X／100」「五段公式」「不出鏡螢幕錄影」全部失效
    （兩份都已加註記）。**仍有效**：定位、受眾、配色模板 A／B、字型、Do／Don't。
  - 💡 **為什麼這個方向對他更好**：他的強項是講話（銷售強），痛點是文書。
    螢幕錄影把他最強的能力關掉了，講話形式打開它。
  - **數字人生成成本實測（2026-08）**：Higgsfield 單支上限都是 15 秒。
    Wan 2.7 ≈ 1.5 credits/秒（30 秒 ≈ 45）、Seedance 2.0 Mini ≈ 2.5/秒、
    Seedance 2.0 ≈ 9/秒。帳上 105.77 credits ≈ 只夠兩支 30 秒。
    **長期做系列建議走 HeyGen／Synthesia 那類訂閱制**，單支便宜且整支一致、不用接片。
  - ⚠️ 生成那一步**雲端 Claude 跑不了**（見下一則的 403 封鎖），要在本機或手機做。

- 2026-08-03：**做出數字人 Reels 產生器**（`tools/digital_human/`），Day 15 成品已交付。
  - 兩種主播共用同一套時間軸／字卡／版面：
    `render_real.py`（**你本人**，從 `IMG_4667.mov` 去背）、`render.py`（向量虛擬人備案）。
  - 腳本 `outputs/scripts/day-15-digital-human.md`，主題「你現在看到的這個人，不是我」。
    這支同時解掉 `brand-style.md` 第 147 行留的問題（不出鏡就沒有臉當視覺錨點）。
  - ⚠️ **這個 repo 是 public。** 所以本人臉部素材（`assets/presenter-*.png`）與
    影片成品（`outputs/videos/`）**已加進 `.gitignore`，不進版控**，只用聊天視窗交付。
    要重做素材跑 `make_presenter.py`。**以後任何本人／客戶影像都比照辦理。**
  - ⚠️ **Higgsfield（AA MCP）在雲端 Claude 這裡用不了**：組織網路政策 403 擋掉
    `upload.higgsfield.ai` 與它的 CDN，照片上傳不上去，所以**雲端沒辦法做 AI 唇形同步**。
    帳上還有 105.77 credits，那是給本機／手機端用的。
  - ⚠️ 真人版的嘴型**只是 2D 下顎變形，不是真唇形同步**，幅度刻意壓小。
    試過合成口內陰影，在閉唇微笑＋頭部傾斜的照片上會變成臉上一塊髒污，已放棄。
  - ⚠️ 去背用 `u2net_human_seg`，它會把**背景路人也當前景**，而且常跟頭髮連在一起，
    連通元件過濾切不開，要用多邊形手動切（座標寫在 `make_presenter.py`）。
  - ✅ **下次拍素材的規格**：正對鏡頭、背景乾淨、光線平均、閉嘴自然表情，拍 5 秒就夠，
    之後 100 天都用同一張。這次三個素材只有一個能用（側臉、手機擋臉的都不行）。

...
- 2026-08-05：**雲端 Claude 每次都是全新容器 —— 空白表單與案件資料都不會留下來。**
  - 填表引擎要 `--form <空白表單.pdf>`，但**空白表單沒有進 git**（全 repo 一個 PDF 都沒有），
    案件 JSON 依隱私規則也只存暫存區。➜ **雲端 session 做不到「沿用之前的資料」**，
    每次都要重新提供空白表單與個資。本機 Claude／Codex 若把檔案留在 C 槽則不受影響。
  - 引擎讀的是**一份合併 PDF**，頁序固定：page 0 = 全球人壽、page 1 = 三商美邦、page 2-3 = 國泰學團險。
    各家分開給的話要先照這個順序合併，否則座標整份對不上。
  - 雲端容器沒有相依套件，要 `pip install pymupdf pypdf`；pypdf 會踩到 Debian 版 cryptography 壞掉，
    補 `pip install --upgrade cffi` 就好。
  - 💡 待使用者決定：要嘛把**空白表單（本身無個資）**放進 repo／私有位置讓雲端也能跑，
    要嘛填表固定在本機做、雲端只負責規則與版面維護。

...
- 2026-07-28：新增「保單保費總表」單檔網頁 `insurance/premium-table-template.html`（+ `README.md`）。
  - 用途：一頁看完一戶人的所有保單 —— 每人明細（公司・商品／要保人／年繳保費／佔比）、
    依**要保人**彙總（誰在繳錢）、依**保險公司**彙總；可改匯率即時重算、可列印存 PDF。
  - 資料就寫在 HTML 最下面「▼▼▼ 保費資料 ▼▼▼」那一塊，改完存檔重整即可，非工程師也能改。
  - **個資做法（重要）**：repo 裡只放**假資料範本**；填了真實姓名的版本放 `private/`，
    已加進 `.gitignore` 不會上傳 —— 對應本文件的安全紅線。
  - 首次使用案例：一戶 4 位被保險人、30 張保單。美元保單用 **1 USD = 32.34** 換算，
    可完全對回使用者提供的四筆年繳總額（尾差 < 1 元）。

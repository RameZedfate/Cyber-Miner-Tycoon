...
| 國泰人壽（個險 300002） | 居住地址標為 (＊) 必填且**無同保單地址選項**，須向使用者索取；日間易晤地址勾「同居住地址」，第二組郵遞區號與地址留空。 | **本文 300002 + 附件 300003 共 2 頁** |
...
- 理賠類別依實際險種勾選；門診手術／實支實付案件勾「醫療實支(F)」，
  無住院天數時**不要順手勾「醫療日額(E)」**。
- ⚠️ **這張跟學團險一樣是兩頁一組，交付要 2 頁**：
  本文 **300002**（事故者基本資料／申請內容）+ 附件 **300003**（保險金給付方式）。
  兩頁都是 115.08 版、右下角都有「個」字圈記。驗證時用 `--expected-pages 2`。
- ⚠️ **本文 300002 沒有領取方式／帳戶欄位**，戶名／身分證字號／金融機構分行／分行通匯代號／帳號
  全部在附件 300003 上。**只拿到本文就先向使用者要附件那頁**，不要只交 1 頁就當作完成。
- 🔎 **怎麼判斷還缺附件：看這張表有沒有「錢要匯去哪」的欄位。**
  一份理賠申請書不可能不寫領取方式，本文頁沒有就代表一定還有附件頁。
  **不要用條碼列的第二組號碼去猜附件編號**——本文頁印的是 `*300002* *00016*`、
  附件頁印的是 `*300003* *00014*`，第二組（00016／00014）是另一套內部碼，
  不是附件的表單編號，照著猜會猜錯（2026-08-10 就猜錯過一次）。

### 國泰人壽 附件 300003（保險金給付方式）specifics

- 領取方式勾「匯款」；禁背支票整組（請服務人員轉送／由申請人親領）留空。
- 帳戶資料有三組，**只填第一組**，勾「匯款至受益人本人帳戶」，第二三組整組留空。
- 第一組要填：戶名、身分證字號（10 格逐格）、金融機構/分行、帳號。
- 「分行通匯代號(7 碼)」未取得就留空（長期規則 5）。
- 金融機構/分行欄只有約 57pt 寬，「○○銀行○○分行」八個字放不下 10 級，靠 `boxes` 自動縮級。
- 個資同意書的立書人簽名、法定代理人簽名、關係，以及「本公司服務人員(送件人)基本資料」
  整區（送件人姓名／單位代號／員編／聯絡電話／收件日期）一律留空。
